...
def get_mfcc_for_one_file_28(path):
    audio, sr = librosa.load(path)
    n_fft = 2048
    hop_length = 512
    frame_length = int(sr * 28)
    ...
    for i in range(frames.shape[1]):
        ...
        mfccs = mfccs.reshape(78, 201)
        mfccs = np.expand_dims(mfccs, axis=-1)
        mfcc_features.append(mfccs)

    mfcc_features = np.array(mfcc_features)  # Convert list to NumPy array
    mfcc_features = mfcc_features.reshape((mfcc_features.shape[0], 78, 201, 1))
    return mfcc_features

def get_mfcc_stft_for_one_file_28(path):
    audio, sr = librosa.load(path)
    n_fft = 2048
    hop_length = 512
    frame_length = int(sr * 28)
    ...
    stfts_features = stfts_features.reshape((stfts_features.shape[0], 78, 201, 1))
